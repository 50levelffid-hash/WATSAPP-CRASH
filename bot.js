...
export default async function initializeTelegramBot(manager) {
  // ── CONFIG ──────────────────────────────────────────────────────────────────
  const ALLOWED_GROUP_ID = Number(process.env.TG_GROUP_ID) || -1003221538912;
  const GROUP_INVITE_LINK =
    process.env.TG_GROUP_LINK || "https://t.me/RTFGMINGGC";
  const WA_CHANNEL_LINK =
    process.env.WA_CHANNEL_LINK ||
    "https://whatsapp.com/channel/0029FnSz8KH3Oqz39";
  const PAIR_COOLDOWN_MS = 30_000; // 30s between pair attempts per user

  const BOT_TOKEN =
    process.env.BOT_TOKEN_TELEGRAM ||
    process.env.BOT_TOKEN ||
    "8196349437:AAEaeOk8dm5G3nzJ96BSVDJEFrBCxN8WdNI";
